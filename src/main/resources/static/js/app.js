...
map.on("load", () => {
    ...
    map.addSource("listedBuildings", {
        type: "geojson",
        data: `/api/listedBuildings/search/findAllInPolygon?polygon=${viewport(
            map
        )}`,
        cluster: true,
        clusterMaxZoom: 14,
    ...
    map.on("moveend", () => {
        ...
        map.getSource("listedBuildings")
            .setData(`/api/listedBuildings/search/findAllInPolygon?polygon=${viewport(map)}`);
    });
});